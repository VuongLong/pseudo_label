...
def test(target_test_loader, custom_clip_model, prompt_list, tokenized_prompts, args, running_means=None):
	...
	with torch.no_grad():
		for data, label in target_test_loader:
			...
			img_feature, feature = custom_clip_model.forward_img_both(data)

			if running_means is not None:
				weights = torch.zeros(4, feature.shape[0]).to(args.device)
				for source_index in range(len(prompt_list)):
					weights[source_index] = torch.norm((feature - running_means[source_index]), p=2, dim=1)
				weights = nn.Softmax(dim=0)(-weights[:-1])
			...
			for source_index, prompt in enumerate(prompt_list):
				...
				if running_means is not None and source_index < 3:
...
def train(domain_list, target_domain, classnames, clip_model, preprocess, args):
	...
	for target_name in domain_list:
		...
		# running_means = torch.zeros(n_domains+1,len(classnames), 1024).to(args.device)
		# running_count = torch.zeros(n_domains+1,len(classnames)).to(args.device)
		running_means = torch.zeros(n_domains+1, 1024).to(args.device)
		running_count = torch.zeros(n_domains+1).to(args.device)
		...
		for step in pbar:
			...
			for source_index in range(n_domains):
				...
				running_means[source_index] = running_means[source_index] * running_count[source_index] + features.sum(0)
				running_count[source_index] += features.shape[0]
				running_means[source_index] /= running_count[source_index]
			...
			running_means[n_domains] = running_means[n_domains] * running_count[n_domains] + features.sum(0)
			running_count[n_domains] += features.shape[0]
			running_means[n_domains] /= running_count[n_domains]

			weights = torch.zeros(n_domains+1, target_img_features.shape[0]).to(args.device)
			for source_index in range(n_domains+1):
			...
			if args.soft_label == 1:
				with torch.no_grad():
					...
					# logits = weights[n_domains].unsqueeze(-1) * (image_features @ text_features.t())
				...
				if step >= args.source_distill_start and args.source_distill == 1:
					for source_index in range(n_domains):
						source_txt_features = text_list[source_index]
						logits +=  weights[source_index].unsqueeze(-1) * (target_img_features @ source_txt_features.t())
					n_pseudo += 1
